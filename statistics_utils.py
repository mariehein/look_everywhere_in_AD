...
def make_pvalues_traintest(filename):
    ...
    return p_value_poissonpoisson(N_after_train, N_samples_after_train/N_samples_train*N_train)

def make_pvalues_kfolds(filename, folds=5):
    BT = np.load(filename+"_BT_preds.npy")
    data = np.load(filename+"_data_preds.npy")
