...
BT_preds = np.zeros((args.runs, args.folds, N_data//10))
data_preds = np.zeros((args.runs, args.folds, N_data//10))

start = time.time()
for i in tqdm.tqdm(range(args.start_runs, args.start_runs+args.runs)):
    idx = np.random.choice(len(data_full), N_data, replace=False)
    data = data_full[idx]

    data, BT =np.array_split(data,2)
    ...
    for k in range(args.folds):
        ...
        if args.classifier=="BDT":
            BT_preds[i-args.start_runs,k], data_preds[i-args.start_runs,k] = cl.BDT_training_and_preds(args, X_train, Y_train, preds_list, i)
        else:
            BT_preds[i-args.start_runs,k], data_preds[i-args.start_runs,k] = cl.NN_training_and_preds(args, X_train, Y_train, preds_list, inputs=inputs)


print("Time taken: ", time.time()-start)
np.save(args.directory+"runs/run"+str(args.start_runs)+"_test_data_preds.npy", data_preds)
np.save(args.directory+"runs/run"+str(args.start_runs)+"_test_BT_preds.npy", BT_preds)